...
namespace TravelReservation.Areas.Admin.Controllers
{
    ...
    public class VisitorApiController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:7904/api/Visitor");
        ...
        public async Task<IActionResult> AddVisitor(VisitorViewModel p)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:7904/api/Visitor", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:7904/api/Visitor/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
